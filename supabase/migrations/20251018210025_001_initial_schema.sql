/*
  ...
  1. New Tables
    ...
    - `food_listings` - Food sharing listings
    - `trades` - Trade/barter transactions
    - `user_stats` - User impact and statistics
    - `user_badges` - User achievements
    - `distribution_events` - Food distribution events
    - `distribution_registrations` - Event registrations
...
CREATE INDEX idx_food_listings_school_district ON food_listings(school_district);
...
CREATE POLICY "Anyone can view public badges" ON user_badges FOR SELECT USING (true);
...
CREATE POLICY "Listing owners can view claims" ON food_claims FOR SELECT USING (EXISTS (
    SELECT 1 FROM food_listings WHERE id = listing_id AND user_id = auth.uid()
));
